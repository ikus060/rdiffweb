...
class rdiffBrowsePage(page_main.rdiffPage):
    ...
    def getParmsForPage(self, repo="", path="", restore=""):
        ...
        parentDirs = []
        parentDirs.append(
            {"parentPath": self.buildBrowseUrl(repo, "/", False), "parentDir": repo.lstrip("/")})
        parentDirPath = "/"
        for parentDir in path.split("/"):
            if parentDir:
                parentDirPath = joinPaths(parentDirPath, parentDir)
                parentDirs.append(
                    {"parentPath": self.buildBrowseUrl(repo, parentDirPath, False), "parentDir": parentDir})
        parentDirs[-1][
        ...
        if restore == "T":
            ...
            tempDates = librdiff.getDirRestoreDates(
                joinPaths(userRoot, repo), path)
            tempDates.reverse()  # sort latest first
            restore_dates = []
            for x in tempDates:
                restore_dates.append(
                    {"url": self.buildRestoreUrl(repo, path, x),
            ...
            for libEntry in libEntries:
                ...
                if libEntry.isDir:
                    ...
                    entryLink = self.buildRestoreUrl(
                        repo, joinPaths(path, libEntry.name), libEntry.changeDates[-1])
                    file_type = "file"
                    entryChangeDates = libEntry.changeDates[:-1]
                    entryChangeDates.reverse()
                    size = libEntry.fileSize
                    change_dates = [
                        {"url": self.buildRestoreUrl(repo, joinPaths(path, libEntry.name), x),
