...
class UserObject(Base):
    ...
    def get_create_or_update_user(cls, login, user_info=None):
        ...
        fullname = str(user_info['fullname']) if user_info and 'fullname' in user_info else None
        email = str(user_info['email']) if user_info and 'email' in user_info else None
...
def update_user_schema(target, conn, **kw):
    ...
    cfg = cherrypy.tree.apps[''].cfg
    if cfg.login_with_email and not index_exists(conn, 'users_email_index'):
