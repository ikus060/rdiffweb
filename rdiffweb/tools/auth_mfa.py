...
MFA_DEFAULT_TRUST_DURATION = 60  # minutes
...
class CheckAuthMfa(cherrypy.Tool):
    ...
    def run(self, mfa_url: str = '/mfa/', mfa_enabled=True, **_):
        ...
        # Optional periodic credential re-check for persistent sessions
        # (Only if your sessions_timeout tool is present and marks persistence)
        requires_relogin = False
        if hasattr(cherrypy.tools, 'sessions_timeout'):
            persistent = cherrypy.tools.sessions_timeout.is_persistent()
            if persistent:
                # Read the session start time directly; fallback to None if absent
                session = cherrypy.serving.session
                start_time = cherrypy.tools.sessions_timeout.get_session_start_time()
                if start_time:
                    # Re-check credentials at the idle timeout cadence
                    idle_minutes = int(cherrypy.request.config.get('tools.sessions.timeout', 60))
                    if (start_time + datetime.timedelta(minutes=idle_minutes)) < session.now():
                        requires_relogin = True

        if requires_relogin:
            cherrypy.tools.auth.clear_login_identity()
            cherrypy.tools.auth.save_original_url()
            raise cherrypy.tools.auth.redirect_to_form_url()
    ...
    def verify_code(self, code: str, persistent: bool = False) -> bool:
        ...
        if not check_password(code, session.get(MFA_CODE)):
            session[MFA_CODE_ATTEMPT] = int(session.get(MFA_CODE_ATTEMPT, 0)) + 1
            return False
